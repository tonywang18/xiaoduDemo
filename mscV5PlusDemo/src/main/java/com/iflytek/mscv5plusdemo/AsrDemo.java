...
//		mgr.asyncQueryAllContactsName();
	...
	private void initLayout(){
		...
		TextView isr_recognize = findViewById(R.id.isr_recognize);
		isr_recognize.setOnClickListener(this);

		TextView isr_grammar = findViewById(R.id.isr_grammar);
		isr_grammar.setOnClickListener(this);

		TextView radioLocal = findViewById(R.id.radioLocal);
		...
		group.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				{
					((EditText)findViewById(R.id.isr_text)).setText("请对小布输入命令,例如：请打开灯光");
					findViewById(R.id.isr_lexcion).setEnabled(true);  //设置本地语法
...
//				mAsr.setParameter(SpeechConstant.SAMPLE_RATE, "8000");
			...
			// 停止识别
		} else if (id == R.id.isr_stop) {
			mAsr.stopListening();
			showTip("停止识别");
			// 取消识别
		} else if (id == R.id.isr_cancel) {
			mAsr.cancel();
			showTip("取消识别");
		}
	}
		...
		public void onResult(final RecognizerResult result, boolean isLast) {
			if (null != result && !TextUtils.isEmpty(result.getResultString())) {
				...
				if (mResultType.equals("json")) {
					...
					Log.d(TAG, "xxxxxxx---json text" + text);
					Bundle bundle = new Bundle();
					bundle.putString("text",text);
					ARouter.getInstance().build("/xfxx/xx/NavDemo").withString("text",text).navigation();
					...
					Log.d(TAG, "xxxxxxx---xml text" + text);
